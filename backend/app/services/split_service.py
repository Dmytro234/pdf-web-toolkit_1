        ...
        try:
            with fitz.open(str(pdf_path)) as doc:
                ...
                for start in range(0, total_pages, pages_per_chunk):
                    ...
                    out_path = (
                        job_dir / f"{pdf_path.stem}_part_{chunk_index}.pdf"
                    )
                    chunk_doc.save(
                        str(out_path),
                        garbage=3,
            ...
            with fitz.open(str(pdf_path)) as doc:
                ...
                while start_page < total_pages:
                    ...
                    final_doc.insert_pdf(
                        doc,
                        from_page=start_page,
                        to_page=last_good_end,
                    )
                    out_path = (
                        job_dir / f"{pdf_path.stem}_size_part_{part_index}.pdf"
                    )
                    final_doc.save(
                        str(out_path),
                        garbage=3,
